...
class NYTimesDataHandler(BaseDataHandler):
    def process_article_links(self, links: List[str]) -> pd.DataFrame:
        """Process and validate NYTimes article links"""
        # Create initial DataFrame
        df = pd.DataFrame({'article_url': links})
        
        # Apply NYTimes-specific filters
        df = df[df['article_url'].str.contains(
            r'/article/|/interactive/|/live/|/2025/',
            regex=True,
            na=False
        )]
        
        # Additional NYTimes-specific validation
        df = df[df['article_url'].str.contains('nytimes.com', na=False)]
