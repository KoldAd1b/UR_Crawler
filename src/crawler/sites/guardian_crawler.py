...
class GuardianCrawler(BaseSiteScraper):
    """Guardian-specific crawler implementation with hybrid pagination"""
    
    def __init__(self, base_crawler):
        config = SiteConfig(
            name="Guardian",
            base_url="https://www.theguardian.com/world/europe-news",
        ...
        self.data_handler = BaseDataHandler(source_name="Guardian")
        self.max_scrolls = 10  # Limit number of scrolls
        self.scroll_count = 0

    def handle_pagination(self) -> bool:
        """Handle Guardian-specific hybrid pagination"""
        try:
            ...
            # If no new content loaded, try the Load More button
    ...
    def handle_dynamic_content(self) -> bool:
        """Handle Guardian-specific dynamic content"""
    ...
    def get_selectors(self) -> Dict[str, str]:
        """Get Guardian-specific selectors"""
        return {
    ...
    async def extract_article_links(self) -> List[str]:
        ...
        try:
            ...
            # Extract links while paginating
            while True:
    ...
    async def extract_article_data(self, url: str) -> Dict[str, Any]:
        """Extract data from a single Guardian article"""
        try:
            self.driver.get(url)
            self.driver.random_delay(1.0, 2.0)
